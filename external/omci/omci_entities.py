...
import json
from scapy.fields import ByteField, ShortField, MACField, BitField, IPField, StrNullField
from scapy.fields import IntField, StrFixedLenField, LongField, FieldListField, PacketLenField, ConditionalField
from scapy.packet import Packet

from external.omci.omci_defs import OmciUninitializedFieldError, \
    AttributeAccess, OmciNullPointer, EntityOperations, OmciInvalidTypeError
from external.omci.omci_fields import OmciSerialNumberField, OmciTableField
from external.omci.omci_defs import bitpos_from_mask
...
class EntityClass(object, metaclass=EntityClassMeta):
...
class PptpPotsUni(EntityClass):
    class_id = 53
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R}),
        ECA(ByteField("administrative_state", 0), {AA.R, AA.W},
            range_check=lambda x: 0 <= x <= 2),
        ECA(ShortField("deprecated", 0), {AA.R, AA.W}, optional = True, deprecated = True),
        ECA(ByteField("arc", 0), {AA.R, AA.W},
            range_check=lambda x: 0 <= x <= 1, avc=True),
        ECA(ByteField("arc_interval", 0), {AA.R, AA.W}, optional = True),
        ECA(ByteField("impedance", 0), {AA.R, AA.W}, optional = True,
            range_check=lambda x: 0 <= x <= 4),
        ECA(ByteField("transmission_path", 0), {AA.R, AA.W}, optional = True,
            range_check=lambda x: 0 <= x <= 1),
        ECA(ByteField("rx_gain", 0), {AA.R, AA.W}, optional = True),
        ECA(ByteField("tx_gain", 0), {AA.R, AA.W}, optional = True),
        ECA(ByteField("operational_state", 0), {AA.R}, optional = True,
            range_check=lambda x: 0 <= x <= 1),
        ECA(ByteField("hook_state", 0), {AA.R}, optional = True,
            range_check=lambda x: 0 <= x <= 1),
        ECA(ShortField("pots_holdover_time", 0), {AA.R, AA.W}, optional = True),
        ECA(ByteField("nominal_feed_voltage", 0), {AA.R, AA.W}, optional = True),
        ECA(ByteField("loss_of_softswitch", 0), {AA.R, AA.W}, optional = True,
            range_check=lambda x: 0 <= x <= 1)
    ]
    mandatory_operations = {OP.Get, OP.Set}
    notifications = {OP.AttributeValueChange}
...
class DesignatedRoot(Packet):
    name = "DesignatedRoot"
    fields_desc = [
        ShortField("root_bridge_priority", None),
        MACField("root_bridge_mac_address", None)
    ]

class MacBridgeConfigurationData(EntityClass):
    class_id = 46
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R}),
        ECA(MACField("bridge_mac_address", None), {AA.R}),
        ECA(ShortField("bridge_priority", None), {AA.R}),
        ECA(PacketLenField("designated_root", None,
                           DesignatedRoot, length_from=lambda pkt: 8), {AA.R}),
        ECA(IntField("root_path_cost", None), {AA.R}),
        ECA(ByteField("bridge_port_count", None), {AA.R}),
        ECA(ShortField("root_port_num", None), {AA.R}),
        ECA(ShortField("hello_time", None), {AA.R},
            range_check=lambda x: 0x0100 <= x <= 0x0A00),
        ECA(ShortField("forward_delay", None), {AA.R},
            range_check=lambda x: 0x0400 <= x <= 0x1E00)
    ]
...
class TcpUdpConfigData(EntityClass):
    class_id = 136
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ShortField("port_id", None), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("protocol", 0x11), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("tos_diffserv_field", 0), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("ip_host_pointer", None), {AA.R, AA.W, AA.SBC})
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}
...
class IPv4MulticastAddress(Packet):
    name = "IPv4MulticastAddress"
    fields_desc = [
        ShortField("gem_port_id", None),
        ShortField("secondary_key",0),

        IPField("ip_multicast_da_range_start", "0.0.0.0"),
        IPField("ip_multicast_da_range_stop", "0.0.0.0")
    ]
    ...
    def index(self):
        return '{:02}'.format(self.fields.get('gem_port_id',0)) + \
               '{:03}'.format(self.fields.get('secondary_key',0))

    def is_delete(self):
        return self.fields.get('ip_multicast_da_range_start',0) == 0x0 and \
            self.fields.get('ip_multicast_da_range_stop',0) == 0x0

    def delete(self):
        self.fields['ip_multicast_da_range_start'] = "0.0.0.0"
        self.fields['ip_multicast_da_range_stop'] = "0.0.0.0"
        return self
...
class MulticastGemInterworkingTp(EntityClass):
    ...
    attributes = [
        ...
        ECA(OmciTableField(
            PacketLenField("ipv4_multicast_address_table", None,
                VlanTaggingOperation, length_from=lambda pkt: 12)), {AA.R, AA.W})
    ]
...
class MulticastOperationsProfile(EntityClass):
    ...
    attributes = [
        ...
        ECA(IPField("querier_ip", None), {AA.R, AA.W, AA.SBC}),
...
class SipUserData(EntityClass):
    class_id = 153
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ShortField("sip_agent_pointer", None), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("user_part_aor", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # LargeString pointer
        ECA(StrFixedLenField("sip_display_name", 0, 25), {AA.R, AA.W}),
        ECA(ShortField("username_and_password", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # Authentication Method pointer
        ECA(ShortField("voicemail_server_sip_uri", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # Network Address Method pointer
        ECA(IntField("voicemail_subscription_expiration_time", 3600), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("network_dial_plan_pointer", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # Network Dial Plan pointer
        ECA(ShortField("voip_application_service_profile_pointer", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # Voip Application Service Profile pointer
        ECA(ShortField("feature_code_pointer", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # Feature Access Code pointer
        ECA(ShortField("pptp_pointer", None), {AA.R, AA.W, AA.SBC}), # POTS TP pointer
        ECA(ByteField("release_timer", 0), {AA.R, AA.W}, optional = True),
        ECA(ByteField("roh_timer", 0xff), {AA.R, AA.W}, optional = True)
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}

class SipAgentConfigData(EntityClass):
    class_id = 150
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ShortField("proxy_server_address_pointer", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # LargeString pointer
        ECA(ShortField("outbound_proxy_address_pointer", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # LargeString pointer
        ECA(IPField("primary_sip_dns", "0.0.0.0"), {AA.R, AA.W, AA.SBC}),
        ECA(IPField("secondary_sip_dns", "0.0.0.0"), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("tcp_udp_pointer", OmciNullPointer), {AA.R, AA.W}), # TCP/UDP Config Data pointer
        ECA(IntField("sip_reg_exp_time", 3600), {AA.R, AA.W}),
        ECA(IntField("sip_rereg_head_start_time", 360), {AA.R, AA.W}),
        ECA(ShortField("host_part_uri", OmciNullPointer), {AA.R, AA.W, AA.SBC}), # LargeString pointer
        ECA(ByteField("sip_status", 0), {AA.R}),
        ECA(ShortField("sip_registrar", None), {AA.R, AA.W, AA.SBC}), # Network Address pointer
        ECA(StrFixedLenField("softswitch", 0, 4), {AA.R, AA.W, AA.SBC}),
        # TODO: need to make table and add column data
        ECA(StrFixedLenField(
            "sip_response_table", None, 5), {AA.R, AA.W}, optional = True),
        ECA(ByteField("sip_option_transmit_control", 0), {AA.R, AA.W, AA.SBC}, optional = True),
        ECA(ByteField("sip_uri_format", 0), {AA.R, AA.W, AA.SBC}, optional = True),
        ECA(ShortField("redundant_sip_agent_pointeh", OmciNullPointer), {AA.R, AA.W, AA.SBC}, optional = True) # Another SIP Agent Config Data pointer
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}

class VoipVoiceCtp(EntityClass):
    class_id = 139
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ShortField("user_protocol_pointer", None), {AA.R, AA.W, AA.SBC}), # SIP User Data pointer
        ECA(ShortField("pptp_pointer", None), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("voip_media_profile_pointer", None), {AA.R, AA.W, AA.SBC}), # VoIP Media Profile pointer
        ECA(ByteField("signalling_code", 1), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: 1 <= x <= 6),
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}

class VoipMediaProfile(EntityClass):
    class_id = 142
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ByteField("fax_mode", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: 0 <= x <= 1),
        ECA(ShortField("voip_service_profile_pointer", None), {AA.R, AA.W, AA.SBC}), # VoIP Service Profile pointer
        ECA(ByteField("codec_selection_1", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: 0 <= x <= 18),
        ECA(ByteField("packet_period_selection_1", 10), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("silence_suppression_selection_1", 1), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("codec_selection_2", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: 0 <= x <= 18),
        ECA(ByteField("packet_period_selection_2", 10), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("silence_suppression_selection_2", 1), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("codec_selection_3", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: 0 <= x <= 18),
        ECA(ByteField("packet_period_selection_3", 10), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("silence_suppression_selection_3", 1), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("codec_selection_4", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: 0 <= x <= 18),
        ECA(ByteField("packet_period_selection_4", 10), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("silence_suppression_selection_4", 1), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("oob_dtmf", 1), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("rtp_profile_pointer", OmciNullPointer), {AA.R, AA.W, AA.SBC}) # RTP Profile pointer
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}

class VoiceServiceProfile(EntityClass):
    class_id = 58
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ByteField("annuncement_type", 0xff), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("jitter_target", 0), {AA.R, AA.W, AA.SBC}, optional=True),
        ECA(ShortField("jitter_buffer_max", 0), {AA.R, AA.W, AA.SBC}, optional=True),
        ECA(ByteField("echo_cancel_ind", 1), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: 0 <= x <= 18),
        ECA(ShortField("pstn_protocol_variant", 0), {AA.R, AA.W, AA.SBC}, optional=True),
        ECA(ShortField("dtmf_digit_levels", 0x8000), {AA.R, AA.W, AA.SBC}, optional=True),
        ECA(ShortField("dtmf_digit_duration", 0), {AA.R, AA.W, AA.SBC}, optional=True),
        ECA(ShortField("hook_flash_minimum_time", 0), {AA.R, AA.W, AA.SBC}, optional=True),
        ECA(ShortField("hook_flash_maximum_time", 0), {AA.R, AA.W, AA.SBC}, optional=True),
        # TODO: need to make table and add column data
        ECA(StrFixedLenField("tone_pattern_table", None, 20), {AA.R, AA.W}, optional = True),
        # TODO: need to make table and add column data
        ECA(StrFixedLenField("tone_event_table", None, 7), {AA.R, AA.W}, optional = True),
        # TODO: need to make table and add column data
        ECA(StrFixedLenField("ringing_pattern_table", None, 5), {AA.R, AA.W}, optional = True),
        # TODO: need to make table and add column data
        ECA(StrFixedLenField("ringing_event_table", None, 7), {AA.R, AA.W}, optional = True),
        ECA(ShortField("network_specific_extensions_pointer", OmciNullPointer), {AA.R, AA.W, AA.SBC}, optional=True) # Network Address pointer
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}
    optional_operations = {OP.SetTable}
    notifications = {OP.AlarmNotification}
...
class RtpProfileData(EntityClass):
    class_id = 143
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ShortField("local_port_min", 50000), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("local_port_max", 0), {AA.R, AA.W, AA.SBC}, optional = True),
        ECA(ByteField("dscp_mark", 0x2e), {AA.R, AA.W, AA.SBC}),
        ECA(ByteField("piggyback_events", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: x <= 0 <= 1),
        ECA(ByteField("tone_events", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: x <= 0 <= 1),
        ECA(ByteField("dtmf_events", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: x <= 0 <= 1),
        ECA(ByteField("cas_events", 0), {AA.R, AA.W, AA.SBC},
            range_check=lambda x: x <= 0 <= 1),
        ECA(ShortField("ip_host_config_pointer", OmciNullPointer), {AA.R, AA.W}, optional = True)
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}

class VoipConfigData(EntityClass):
    class_id = 138
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R}),
        ECA(ByteField("available_signalling_protocols", 0), {AA.R}),
        ECA(ByteField("signalling_protocol_used", 1), {AA.R, AA.W}),
        ECA(IntField("available_voip_configuration_methods", None), {AA.R}),
        ECA(ByteField("voip_configuration_method_used", 1), {AA.R, AA.W}),
        ECA(ShortField("voip_configuration_address_pointer", OmciNullPointer), {AA.R, AA.W}),
        ECA(ByteField("voip_configuration_state", None), {AA.R}),
        ECA(ByteField("retrieve_profile", None), {AA.W}),
        ECA(StrFixedLenField("profile_version", None, 25), {AA.R})
    ]
    mandatory_operations = {OP.Get, OP.Set}

class LargeString(EntityClass):
    class_id = 157
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ByteField("number_of_parts", 0), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_1", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_2", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_3", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_4", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_5", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_6", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_7", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_8", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_9", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_10", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_11", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_12", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_13", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_14", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("part_15", 0, 25), {AA.R, AA.W})
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}
    ...
    def update_string(string):
        ...
        data['number_of_parts'] = math.ceil(len(string) / 25)

        while ((len(string) - start) > 0):
            start += 25
            data[f'part_{i}'] = string[start - 25:start]
            i += 1

        return data

class NetworkAddress(EntityClass):
    class_id = 137
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ShortField("security_pointer", None), {AA.R, AA.W, AA.SBC}),
        ECA(ShortField("address_pointer", None), {AA.R, AA.W, AA.SBC})
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}

class AuthenticationSecurityMethod(EntityClass):
    class_id = 148
    attributes = [
        ECA(ShortField("managed_entity_id", None), {AA.R, AA.SBC}),
        ECA(ByteField("validation_scheme", 0), {AA.R, AA.W},
            range_check=lambda x: 0 <= x <= 3),
        ECA(StrFixedLenField("username_1", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("password", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("realm", 0, 25), {AA.R, AA.W}),
        ECA(StrFixedLenField("username_2", 0, 25), {AA.R, AA.W})
    ]
    mandatory_operations = {OP.Create, OP.Delete, OP.Get, OP.Set}
...
entity_classes = [c for c in iter(entity_classes_name_map.values())]
