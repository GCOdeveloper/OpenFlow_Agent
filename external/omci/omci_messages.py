...
from external.omci.omci_defs import AttributeAccess, OmciSectionDataSize
from external.omci.omci_fields import OmciTableField, OmciVariableLenZeroPadField
import external.omci.omci_entities as omci_entities
...
class OmciMaskedData(Field):
    ...
    def addfield(self, pkt, s, val):
        ...
        entity_class = omci_entities.entity_id_to_class_map.get(class_id)
        indices = entity_class.attribute_indices_from_mask(attribute_mask) if entity_class is not None else []
    ...
    def getfield(self, pkt, s):
        ...
        for index in indices:
            try:
                ...
                logging.error("attribute-decode-failure attribute index: %d, Class ID: %d, Error: %s",
                              index, entity_class.class_id, str(e))
                continue
            try:
                s, value = fld.getfield(pkt, s)
